 ...
 */
package me.entity303.antipluginlookup.metrics;
...
public class MetricsLite {
    ...
    public MetricsLite(JavaPlugin plugin, int serviceId) {
        ...
        var serverUUID = config.getString("serverUuid");
        var logErrors = config.getBoolean("logFailedRequests", false);
        var logSentData = config.getBoolean("logSentData", false);
        var logResponseStatusText = config.getBoolean("logResponseStatusText", false);
    ...
    public static class MetricsBase {
        ...
        public MetricsBase(String platform, String serverUuid, int serviceId, boolean enabled,
                           Consumer<JsonObjectBuilder> appendPlatformDataConsumer, Consumer<JsonObjectBuilder> appendServiceDataConsumer,
                           Consumer<Runnable> submitTaskConsumer, Supplier<Boolean> checkServiceEnabledSupplier,
                           BiConsumer<String, Throwable> errorLogger, Consumer<String> infoLogger, boolean logErrors, boolean logSentData,
                           boolean logResponseStatusText) {
            var scheduler = new ScheduledThreadPoolExecutor(1, task -> new Thread(task, "bStats-Metrics"));
        ...
        private void checkRelocation() {
            ...
            if (System.getProperty("bstats.relocatecheck") == null || !System.getProperty("bstats.relocatecheck").equals("false")) {
                ...
                // little "trick" ... :D
                final var defaultPackage = new String(new byte[] { 'o', 'r', 'g', '.', 'b', 's', 't', 'a', 't', 's' });
        ...
        private void startSubmitting() {
            ...
            // don't do it!
            var initialDelay = (long) (1000 * 60 * (3 + Math.random() * 3));
            var secondDelay = (long) (1000 * 60 * (Math.random() * 30));
        ...
        private void submitData() {
            ...
            var data = baseJsonBuilder.build();
        ...
        private void sendData(JsonObjectBuilder.JsonObject data) throws Exception {
            ...
            var url = String.format(REPORT_URL, this.platform);
            var connection = (HttpsURLConnection) new URL(url).openConnection();
            // Compress the data to save bandwidth
            var compressedData = compress(data.toString());
    ...
    /**
     ...
     for its use-case.
     */
    @SuppressWarnings("UnnecessaryUnicodeEscape") public static class JsonObjectBuilder {
        ...
        public void appendField(String key, JsonObject... values) {
            ...
            var escapedValues = Arrays.stream(values).map(JsonObject::toString).collect(Collectors.joining(","));
        ...
        public JsonObject build() {
            if (this.builder == null)
                throw new IllegalStateException("JSON has already been built");
            var object = new JsonObject(this.builder.append("}").toString());
            this.builder = null;
            return object;
        }
